...
exports.buildDeductionSummaryMarkdown = buildDeductionSummaryMarkdown;
exports.buildDeductionsRawExport = buildDeductionsRawExport;
exports.extractRolloutMeta = extractRolloutMeta;
function parseRewardResponseObject(rr) {
    if (rr == null) {
        return null;
    }
...
function getRewardContainer(rollout) {
...
function getRewardInfoObject(rollout) {
...
function sliceAfterRedactedThinking(raw) {
...
function sliceAfterThinking(raw) {
...
function extractDeductionsInners(text) {
...
function extractEvaluationInners(text) {
    const s = sliceAfterRedactedThinking(sliceAfterThinking(text == null ? '' : String(text)));
...
function normalizeFieldKeyName(key) {
    return key.toLowerCase().replace(/[\s_-]+/g, ' ').trim();
}
function parseEvaluationFields(inner) {
    const fields = {};
    if (!inner) {
        return fields;
    }
...
function parseDeductionBlockFields(inner) {
    const fields = {};
    if (!inner) {
        return fields;
    }
 ...
 */
function buildDeductionSummaryMarkdown(rollout) {
    ...
    const trajBlocks = trajDedInners.length > 0
    ...
    const ansBlocks = ansDedInners.length > 0
    ...
    if (fr != null) {
        lines.push('- **综合 reward**：' + (typeof fr === 'number' ? fr.toFixed(3) : String(fr)));
    }
    lines.push('');
    const fieldLabels = [
        ['dimension', '扣分维度'],
    ...
    function emitStep(sectionZh, idx, fields, kind) {
    ...
    if (trajBlocks.length === 0) {
        ...
        for (let t = 0; t < trajBlocks.length; t++) {
            emitStep('轨迹', t + 1, trajBlocks[t].fields, trajBlocks[t].kind);
        }
    }
    lines.push('---');
    lines.push('');
    ...
    if (ansBlocks.length === 0) {
        ...
        for (let a = 0; a < ansBlocks.length; a++) {
            emitStep('回答', a + 1, ansBlocks[a].fields, ansBlocks[a].kind);
        }
    }
    ...
    lines.push('');
    lines.push('*本报告由 Viewer 根据 `reward_response.raw_outputs` 中的 `<deductions>` / `<evaluation>` 自动解析生成。*');
    lines.push('');
    return lines.join('\n');
}
/**
 ...
 */
function buildDeductionsRawExport(rollout) {
    const ro = getRawOutputsFromRollout(rollout);
    if (!ro) {
        return null;
    }
    const trajDedBlocks = extractDeductionsInners(ro.trajectory).map(inner => '<deductions>' + inner + '</deductions>');
    const ansDedBlocks = extractDeductionsInners(ro.answer).map(inner => '<deductions>' + inner + '</deductions>');
    const trajEvalBlocks = extractEvaluationInners(ro.trajectory).map(inner => '<evaluation>' + inner + '</evaluation>');
    const ansEvalBlocks = extractEvaluationInners(ro.answer).map(inner => '<evaluation>' + inner + '</evaluation>');
    ...
    const lines = [];
    lines.push('raw_outputs.trajectory — ' + (trajDedBlocks.length > 0 ? '<deductions>' : '<evaluation>'));
    lines.push('');
 ...
 */
function extractRolloutMeta(rollout) {
    ...
    if (rr?.scores && typeof rr.scores === 'object' && !Array.isArray(rr.scores)) {
        ...
        if (typeof sc.answer === 'number') {
            answer_score = sc.answer;
        }
    }
    return { question, request_id, score, trajectory_score, answer_score, final_reward };
}
//# sourceMappingURL=deductionReport.js.map